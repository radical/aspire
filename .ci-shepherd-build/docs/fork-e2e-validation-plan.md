...
| C14 | Exact `microsoft/aspire` edit proposal from a finalized round-zero or round-one snapshot collected less than 15 minutes earlier | Request the production pilot | The finalized proposal document carries a digest-bound production capability; grant creation and execution each require their own flag, and the grant expires no later than 15 minutes after collection | Any provisional document, create, multiple-action, closure, dependency, suppression override, stale snapshot, round mismatch, flag mismatch, or broadened grant is rejected; the actor independently rejects non-comment endpoints | none | A nominal pilot switch lifting unrelated production guards (T1, T2) | L |
...
Rows H1-H4, H5b, H8-H11, H13-H14, H16-H19, H22-H27, and H30 are
deterministic local gates today.
The remaining mutation, push, pull-request, and merged-tree rows must pass before
production quarantine can be enabled.
...
| H18 | Checkout source is edited while the inspector process is running, without changing HEAD | Every candidate is excluded | The pre- and post-inspection source digests differ; all candidates receive `source-inspection-unavailable` and no grantable batch remains | Restore worktree | A resolution being bound to source bytes other than those the inspector actually read | L |
| H19 | Inspector source or a repository build input changes after inspection but before grant use | Authorization fails | The current source-input digest differs from `sourceTreeDigest`; no mutation process starts | Restore worktree | An inspector or build-input change being silently masked by a commit-only provenance check | L |
| H20 | Eight quarantine candidates in one cycle | One inspection process | Exactly one inspector invocation is recorded, with eight entries in its response; the per-candidate results match a per-candidate control run | none | N process launches per cycle, and divergence between batched and single-name resolution | L |
| H21 | An unchanged candidate is reviewed in two consecutive cycles | Two fresh inspections | Each cycle records one inspector invocation and independently binds its result to the current source-input digest | none | Undocumented stale cache state influencing source eligibility | L |
