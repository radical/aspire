...
from .investigation_scope import diagnostic_get_contract, validate_reproduction_commands, validate_scoped_result, validate_work_log
...
_MAX_INVESTIGATIONS_PER_CYCLE = 5
_IDENTICAL_VALIDATION_FAILURE_LIMIT = 2
...
def _worker_prompt(request: Mapping[str, Any]) -> str:
    ...
    permissions = (
        ...
        "Use this exact GET contract, generated by the result validator:\n"
        f"{json.dumps(diagnostic_get_contract(request), indent=2)}\n"
        f"Example: gh api --method GET 'https://api.github.com/repos/{request['repository']}/issues/{request['issueNumber']}'\n"
        "Do not invoke the broader issue-investigation skill: its discovery routes "
        "are not authorized here. If a URL does not match this contract, use the "
        "embedded evidence or pinned local source, or report the missing fact.\n\n"
    ...
    return (
        ...
        + (
            ...
            f"/issues/{request['issueNumber']}\",\"finding\":\"the observed fact\"}}. Reproduction entries: "
...
def build_investigation_plan(
    ...
    *,
    max_requests: int = _MAX_INVESTIGATIONS_PER_CYCLE,
) -> dict[str, object]:
    ...
    for issue in judgments.get("issues", []):
        ...
        for recommendation in issue.get("recommendations", []):
            ...
            if source_revision is not None:
                ...
                request["resultValidationProtocol"] = 1
    ...
    for index, request in enumerate(requests):
        if request.get("investigationScope") is not None:
            # Admission consumes the same frozen ordering as planning, rather
            # than letting a launcher select a lower-priority case by ID.
            request["admissionPredecessors"] = [row["investigationId"] for row in requests[:index]]
    reused.sort()
    active.sort()
...
def _validations_path(state_directory: Path) -> Path:
    return state_directory / "ledgers" / "investigation-validations.jsonl"


def _record_result_validation(
    state_directory: Path, request: Mapping[str, Any], previous: Mapping[str, Any],
    result: Mapping[str, Any], *, recorded_at: str, error: ValueError | None,
    runtime_observation: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Append actual validator observations under the caller's session lock."""
    event = {
        "schemaVersion": 1, "repository": request["repository"], "snapshotId": request["snapshotId"],
        "sourceRevision": request["sourceRevision"], "investigationId": request["investigationId"],
        "ownershipId": previous["worktreeOwnershipId"], "attemptId": previous.get("attemptId"),
        "sessionId": previous.get("sessionId"), "requestFingerprint": _fingerprint(dict(request)),
        "responseFingerprint": _fingerprint(dict(result)), "validatedAt": recorded_at,
        "valid": error is None, "error": str(error) if error is not None else None,
        "errorCode": _fingerprint(str(error)) if error is not None else None,
        "runtimeObservation": dict(runtime_observation) if runtime_observation is not None else None,
    }
    path = _validations_path(state_directory)
    # Exact retries of the same invalid response are one observation, not
    # multiple failing workers and not grounds for opening the circuit.
    for existing in read_jsonl_rows(path):
        if {key: value for key, value in existing.items() if key != "validatedAt"} == {
            key: value for key, value in event.items() if key != "validatedAt"
        }:
            return existing
...
def _validate_result_circuit(state_directory: Path, request: Mapping[str, Any]) -> list[dict[str, Any]]:
    validations = [
        row for row in read_jsonl_rows(_validations_path(state_directory))
        if row.get("repository", "").casefold() == request["repository"].casefold()
        and row.get("snapshotId") == request["snapshotId"]
        and row.get("sourceRevision") == request.get("sourceRevision")
    ]
    failures: dict[str, set[str]] = {}
    for row in validations:
        if row["valid"] is False:
            failures.setdefault(row["errorCode"], set()).add(row["ownershipId"])
    if any(len(owners) >= _IDENTICAL_VALIDATION_FAILURE_LIMIT for owners in failures.values()):
        raise ValueError("Investigation result-validation circuit is open: two attempts failed the same structural validation in this cycle.")
    return validations


def _validate_first_result_preflight(
    state_directory: Path, request: Mapping[str, Any], history: list[Mapping[str, Any]],
) -> None:
    validations = _validate_result_circuit(state_directory, request)
    if request.get("resultValidationProtocol") != 1:
        return
    if any(row["valid"] is True for row in validations):
        return
    dispatched = [
        row for row in history
        if row.get("repository", "").casefold() == request["repository"].casefold()
        and row.get("request", {}).get("snapshotId") == request["snapshotId"]
        and row.get("status") == "dispatching"
    ]
    if not dispatched:
        return
    for first in dispatched:
        owner = first["worktreeOwnershipId"]
        if any(row["ownershipId"] == owner for row in validations):
            continue
        terminal = next((row for row in reversed(history) if row.get("worktreeOwnershipId") == owner), first)
        if terminal.get("workerStopped") is True and terminal.get("executionState") == "not-launched":
            continue
        raise ValueError("Preflight the first dispatched investigation's actual result before dispatching more workers; its validation is still unknown.")


def _runtime_observation(value: Mapping[str, Any] | None, recorded_at: str) -> dict[str, Any] | None:
    if value is None:
        return None
    fields = {"runtimeSessionId", "workerStartedAt", "workerCompletedAt", "observationEvidence"}
    if not isinstance(value, Mapping) or set(value) != fields:
        raise ValueError("Runtime observation must contain the exact runtime identity, start/end and evidence fields.")
    identity = value["runtimeSessionId"]
    if identity is not None and (not isinstance(identity, str) or not identity.strip() or len(identity) > 500):
        raise ValueError("Runtime session ID must be an observed bounded identifier or null.")
    evidence = value["observationEvidence"]
    if not isinstance(evidence, str) or not evidence.strip() or len(evidence) > 4000:
        raise ValueError("Runtime timing/identity requires observed launcher or runtime evidence, not worker self-report.")
    end = parse_aware_iso8601(recorded_at, "recordedAt")
    started = value["workerStartedAt"]
    completed = value["workerCompletedAt"]
    for name in ("workerStartedAt", "workerCompletedAt"):
        if value[name] is not None and parse_aware_iso8601(value[name], name) > end:
            raise ValueError("Observed worker timing cannot be after its recording time.")
    if started is not None and completed is not None and (
        parse_aware_iso8601(started, "workerStartedAt") > parse_aware_iso8601(completed, "workerCompletedAt")
    ):
        raise ValueError("Observed worker completion cannot precede its start.")
    return dict(value)
...
def record_investigation_session_event(
    ...
    *,
    ...
    observation = _runtime_observation(runtime_observation, recorded_at)
    if observation is not None and status not in {"failed", "abandoned"}:
        raise ValueError("Record runtime observations with terminal outcomes, not preparation or dispatch intent.")
    if observation is not None and request.get("investigationScope") is None:
        raise ValueError("Runtime observations require a source-scoped owned worker.")
...
def _validate_result_shape(
    request: Mapping[str, Any], result: Mapping[str, Any], attempt_id: str | None,
) -> Mapping[str, Any]:
...
def record_investigation_result(
    ...
    *,
    ...
    if type(preflight_only) is not bool:
        raise ValueError("Result preflight must be an explicit boolean.")
    if (preflight_only or observation is not None) and request.get("investigationScope") is None:
        raise ValueError("Result preflight and runtime observations require a source-scoped owned worker.")
    if attempt_id is not None:
    ...
    try:
        result = _validate_result_shape(request, result, attempt_id)
    except ValueError as error:
        if request.get("investigationScope") is not None:
            list_investigation_worktrees(state_directory)
            with exclusive_jsonl_lock(_sessions_path(state_directory)):
                previous = _latest_session_event(
                    read_investigation_session_events(state_directory),
                    repository=request["repository"], investigation_id=request["investigationId"],
                )
                _scoped_session_binding(state_directory, request, previous, checkout, session_id, attempt_id)
                if previous["status"] in {"started", "dispatching"}:
                    _record_result_validation(
                        state_directory, request, previous, result, recorded_at=recorded_at,
                        error=error, runtime_observation=observation,
                    )
        raise
    investigation_id, repository = _investigation_identity(request)
    event: dict[str, object] = {
        ...
        "evidenceIds": sorted(set(result["evidenceIds"])),
        "reassessWhen": result["reassessWhen"],
        "recordedAt": recorded_at,
        "sessionId": session_id,
    }
    if isinstance(result.get("fixHandoff"), Mapping):
        event["fixHandoff"] = dict(result["fixHandoff"])
    ...
    if request.get("investigationScope") is not None:
        return _record_scoped_result(
            ...
            execution_evidence=execution_evidence, confirm_worker_stopped=confirm_worker_stopped,
            preflight_only=preflight_only, runtime_observation=observation,
        )
...
def _validate_investigation_limits(
    ...
    _validate_result_circuit(state_directory, request)
    ...
    if sum(row.get("request", {}).get("snapshotId") == request.get("snapshotId") for row in registrations) >= _MAX_INVESTIGATIONS_PER_CYCLE:
        raise ValueError("Five investigation attempts are already reserved in this cycle.")
    predecessors = request.get("admissionPredecessors", [])
    if (
        not isinstance(predecessors, list) or len(predecessors) >= _MAX_INVESTIGATIONS_PER_CYCLE
        or any(not isinstance(value, str) or not value for value in predecessors)
        or len(set(predecessors)) != len(predecessors) or investigation_id in predecessors
    ):
        raise ValueError("Investigation admission order must match the bounded canonical plan.")
    admitted = {
        row["investigationId"] for row in registrations
        if row.get("request", {}).get("snapshotId") == request.get("snapshotId")
    }
    # A higher-priority allocation rejected before launching must be durably
    # reconciled before the next item can advance; missing workers never count.
    admitted.update(
        row["investigationId"] for row in list_investigation_worktrees(state_directory)
        if row["repository"].casefold() == repository.casefold()
        and row["request"].get("snapshotId") == request.get("snapshotId")
        and row.get("launchOutcome") in {"registration-rejected", "not-invoked"}
        and row.get("workerStopped") is True
    )
    if any(value not in admitted for value in predecessors):
        raise ValueError("A higher-priority investigation in the canonical plan must be admitted or reconciled first.")
...
def _record_one_shot_session(
    ...
    with exclusive_jsonl_lock(path):
        ...
        if status == "dispatching":
            ...
            _validate_first_result_preflight(state_directory, request, history)
        ...
        if runtime_observation is not None:
            if execution_state != "returned":
                raise ValueError("Observed worker timing cannot accompany an unlaunched or unknown execution.")
            event["runtimeObservation"] = dict(runtime_observation)
            event["runtimeSessionId"] = runtime_observation["runtimeSessionId"]
            event["workerIdentityKind"] = "runtime-session" if event["runtimeSessionId"] else "unknown"
...
def _record_scoped_session_event(
    ...
    *,
    ...
    with exclusive_jsonl_lock(path):
        ...
        if runtime_observation is not None:
            event["runtimeObservation"] = dict(runtime_observation)
        elif "runtimeObservation" in previous:
            event["runtimeObservation"] = previous["runtimeObservation"]
        if previous.get("status") == status:
    ...
    *,
    ...
    with exclusive_jsonl_lock(sessions_path):
        ...
        with exclusive_jsonl_lock(results_path):
            ...
            if existing is not None:
                for field in ("acceptedResultAt", "runtimeObservation", "runtimeSessionId", "workerIdentityKind"):
                    if field in existing:
                        event[field] = existing[field]
                if runtime_observation is not None:
                    event["runtimeObservation"] = dict(runtime_observation)
                    event["runtimeSessionId"] = runtime_observation["runtimeSessionId"]
                    event["workerIdentityKind"] = "runtime-session" if event["runtimeSessionId"] else "unknown"
                ...
                if preflight_only:
                    return {
                        "valid": True, "validatedAt": existing["recordedAt"],
                        "acceptedResultAt": existing.get("acceptedResultAt"),
                ...
                try:
                    ...
                    raise
                validation = _record_result_validation(
                    state_directory, request, previous, result, recorded_at=recorded_at, error=None,
                    runtime_observation=runtime_observation,
                )
                if preflight_only:
                    return validation
                event["acceptedResultAt"] = recorded_at
                event["runtimeObservation"] = dict(runtime_observation) if runtime_observation is not None else None
                if runtime_observation is not None:
                    event["runtimeSessionId"] = runtime_observation["runtimeSessionId"]
                    event["workerIdentityKind"] = "runtime-session" if event["runtimeSessionId"] else "unknown"
            completed = {
