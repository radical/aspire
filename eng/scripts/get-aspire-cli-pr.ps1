...
# joined into $ResolvedInstallPrefix\hives\<channel> and then passed to
# Remove-Item -Recurse, so any path separator or '..' segment would let the
# removal escape the hives directory. Mirrors localhive.ps1's Test-HiveName
# and the CLI's CliCleanupService.IsValidHiveName.
