...
function Test-FileChecksum {
    ...
    try {
        ...
        if ($expectedChecksum -ne $actualChecksum) {
            $displayExpected = if ($expectedChecksum.Length -gt 32) {
                $expectedChecksum.Substring(0, 32) + "..."
            }
            else {
                $expectedChecksum
            }
            $displayActual = if ($actualChecksum.Length -gt 32) {
                $actualChecksum.Substring(0, 32) + "..."
            }
            else {
                $actualChecksum
            }

            throw "Checksum validation failed for $ArchiveFile with checksum from $ChecksumFile`nExpected: $displayExpected`nActual:   $displayActual"
        }

        Write-Message "Checksum validation successful" -Level Verbose
    }
    catch {
        if ($_.Exception.Message -like "*Checksum validation failed*") {
            throw
        }
        throw "Failed to validate checksum: $($_.Exception.Message)"
    }
}
