...
**⚠️ Do NOT use the `create-pr` skill or the repository's generic PR template.** This skill has its own specific PR template below. Using a different template risks including `Fixes #` keywords that auto-close the tracking issue.
...
### 6.2.1: ⛔ Validate the Created PR (MANDATORY)

**This step is a hard gate. Do not proceed to 6.3 until all validations pass.**

After opening the final PR, **programmatically validate** that the PR body and diff are correct. Do NOT skip this step — it catches mistakes that passive checklists miss.

```bash
# Fetch the PR body and diff for validation
PR_BODY=$(gh pr view <pr-number> --repo dotnet/aspire --json body --jq '.body')
PR_DIFF=$(gh pr diff <pr-number> --repo dotnet/aspire)
```

Run **all** of these checks. If ANY check fails, fix the PR immediately with `gh pr edit` before proceeding:

**1. No auto-close keywords** — The PR must NOT contain GitHub auto-close keywords that would close the tracking issue on merge:
```bash
# MUST return empty — if it matches, the PR will incorrectly close the issue on merge
echo "$PR_BODY" | grep -iE '^\s*(fix(es|ed)?|close[sd]?|resolve[sd]?)\s+#' || echo "✅ No auto-close keywords"
```
If this check fails: edit the PR body to remove the `Fixes #`, `Closes #`, or `Resolves #` line. Replace with a reference like `Related to #<issue-number>` or ensure the "intentionally does not close" note is present.

**2. Required sections present**:
```bash
echo "$PR_BODY" | grep -q "intentionally does not close" || echo "❌ Missing 'intentionally does not close' note"
echo "$PR_BODY" | grep -q "QuarantinedTest" || echo "❌ Missing QuarantinedTest retention note"
echo "$PR_BODY" | grep -q "Root Cause" || echo "❌ Missing Root Cause section"
echo "$PR_BODY" | grep -q "Verification" || echo "❌ Missing Verification section"
echo "$PR_BODY" | grep -qE "actions/runs/[0-9]+" || echo "❌ Missing CI run links"
```

**3. No workflow files in diff** — The clean fix PR must not contain workflow changes:
```bash
echo "$PR_DIFF" | grep -q "^diff.*\.github/workflows/" && echo "❌ Workflow files in PR diff" || echo "✅ No workflow files"
```

**4. QuarantinedTest attribute still present** — The diff must not remove the quarantine attribute:
```bash
echo "$PR_DIFF" | grep -q "^-.*QuarantinedTest" && echo "❌ QuarantinedTest attribute removed" || echo "✅ QuarantinedTest preserved"
```

**If any validation fails**, fix it immediately:
```bash
# Example: fix auto-close keyword in PR body
gh pr edit <pr-number> --repo dotnet/aspire --body "<corrected body>"
```

Then re-run the validation checks to confirm all pass before proceeding.
...
Before marking the task complete, verify **every item**. Items marked with 🤖 are automatically checked by Step 6.2.1 — but review them anyway.
...
- [ ] 🤖 **PR body does NOT contain auto-close keywords** (`Fixes #`, `Closes #`, `Resolves #`) — validated by Step 6.2.1
- [ ] 🤖 **PR body contains "intentionally does not close" note** — validated by Step 6.2.1
- [ ] 🤖 **`[QuarantinedTest]` attribute is still present** on the test method (not removed) — validated by Step 6.2.1
- [ ] **Tracking issue is still open** (not closed)
- [ ] 🤖 Clean fix PR is open with only code changes (no workflow modifications) — validated by Step 6.2.1
- [ ] **Git history is clean**: only fix-related commits, no leftover config/revert pairs (squash if needed)
