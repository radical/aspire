...
test("visibleCheckState and isChecksFailing honor non-blocking check rules", () => {
  ...
  const aggregateFailure = makePr({ repository: "dotnet/aspire.1p", checks: { state: "failure" } });
...
test("filterCheckFailureRules drops rules missing a repository, label, or concrete matcher", () => {
  const valid = { repository: "dotnet/aspire.1p", label: "proof of presence", checkNames: ["GitOps/GitHubPop"], checkNameContains: [] };
